...
function EditBook() {
    // Context
    const { loading, setLoading, notify, rolling, setRolling } = useBooksContext();
    ...
    const deleteFile = (index) => {
        if (!(files[index] instanceof Blob) && files[index].includes("uploads")) {
    ...
    useEffect(() => {
        ...
        const asyncFunction = async () => {
            ...
            if (response.status === 200) {
                response.json.tags = response.json.tags.toString().replace(/[,]/g, " ");
                response.json.publishDate = response.json.publishDate.substring(0, 10);
    ...
    return (
        <>
            ...
            {loading && <Loader />}
            {!loading && <form onSubmit={formSubmitHandle} className="row mb-5 g-3 text-light">
                ...
                <div className="col-md-3">
                    ...
                    <input type="date" value={book.publishDate ? book.publishDate.substring(0, 10) : ""} className="form-control text-light bg-dark" name="publishDate" id="publishDate" onChange={onChangeHandle} />
                </div>
                <div className="col-md-2">
                    <label htmlFor="pages" className="form-label">Pages</label>
                    <input type="text" value={book.pages} className="form-control text-light bg-dark" name="pages" id="pages" onChange={onChangeHandle} />
                </div>
                <div className="col-md-2">
                    <label htmlFor="price" className="form-label">Price (PKR)</label>
                    <input type="text" value={book.price} className="form-control text-light bg-dark" name="price" id="price" onChange={onChangeHandle} />
                </div>
                ...
                <div className="col-12">
                    <button disabled={rolling} type="submit" className="btn btn-primary d-flex justify-content-between align-items-center" style={{ width: "10rem" }}>
                        <span className="d-inline-flex">
                            <img src={rolling ? Rolling : CloudArrowUp} style={{ width: "20px" }} alt={rolling ? "loading" : "add-book"} />
                        </span> Update Book
                    </button>
                </div>
            </form>}
